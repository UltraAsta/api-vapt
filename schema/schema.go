...
type Findings struct {
	Endpoint string // the path
	Method   string // HTTP method used
	Attack   string // e.g. "
	Severity string // "critical", "high", "medium", "low", "info"
	Evidence string // what triggered it — response snippet, status code diff, etc.
	Request  string // the exact request that triggered it
	Response string // the response that confirmed it
}
